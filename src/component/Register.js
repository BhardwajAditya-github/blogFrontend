...
const Register = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const res = await axios.post(`https://blogserver-que3.onrender.com/signup`, { name, email, password })
        ...
        <Layout>
            <div className='signup'>
                <form onSubmit={handleSubmit}>
                    <div className='signup-input'><label for="name">Name</label><input id='name' type='text' placeholder='Enter your name' value={name} onChange={(e) => { setName(e.target.value) }}></input></div>
                    ...
                    <div className='signup-input' style={{ backgroundColor: "#00000000" }}><button type='submit' className='signup-btn'>Signup</button></div>
                </form>
            </div>
        </Layout>
    )
